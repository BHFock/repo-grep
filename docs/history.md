...
A weekend with Google and StackOverflow answered that question. I learned that Emacs wasn’t just an editor. It was programmable. I figured out how to grab the [symbol under the cursor](http://xahlee.info/emacs/emacs/elisp_thing-at-point.html) and pass it into a search command. That first hack became a small tool: one keystroke to search for a symbol across the whole project. It kept me in the flow and quickly became part of my everyday toolkit.
